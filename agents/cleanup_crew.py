...
The Deletor asks questions:
- Is this agent still useful?
- Can memory be compressed?
- Should results be archived?
- Should another agent inherit this context?
- What's the memory/CPU cost of keeping it?

This is what makes HIVE feel like a real OS, not just a script.
"""
...
from core.agent_state import agent_registry, get_or_create_state
...
class CleanupDecision:
    """A decision made by the cleanup crew."""

    agent_id: str
    action: str  # "keep", "archive_and_keep", "archive_and_delete", "delete"
    reason: str
...
class CleanupCrew:
    """
    Intelligent garbage collector for the agent society.

    Questions it asks:
    1. Is the agent still working on something?
    2. Are its results valuable enough to keep?
    3. Is memory pressure high enough to force cleanup?
    4. Can other agents benefit from its context?
    5. What's the cost of keeping vs deleting?
    """
    ...
    def _evaluate_cleanup(self, agent_id: str, state, is_idle: bool, is_orphan: bool, memory_critical: bool) -> CleanupDecision:
        ...
        from agents.agent_forge import temporary_agents
    ...
    def _estimate_memory_freed(self, agent_id: str, state) -> int:
        base = 50
        task_memory = state.tasks_completed * 10
        return base + task_memory
    ...
    def archive_agent(self, agent_id: str, state) -> Optional[str]:
        ...
        archive_data = {
            ...
            "stress_history": state.stress_history[-20:],
    ...
    def delete_agent(self, agent_id: str) -> bool:
        ...
        from agents.agent_forge import temporary_agents, AGENT_REGISTRY
    ...
    def should_archive_task_result(self, result_data: dict) -> bool:
        confidence = result_data.get("confidence", 0)
        agent_id = result_data.get("agent_id", "unknown")
        state = get_or_create_state(agent_id)
        if confidence > 0.8 and state.reputation > 0.6:
            return True
    ...
    def get_cleanup_status(self) -> dict:
        ...
        return {
            ...
            "archive_dir_size_mb": self._get_archive_size(),
        }
