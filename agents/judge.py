...
Usage:
    from agents.judge import judge_action
...
def _is_high_stakes(action: str) -> bool:
    """Check if action involves high monetary value."""
    import re
...
def _has_risk_flags(action: str) -> bool:
    """Check for explicit risk language."""
...
async def judge_action(
    ...
    """
    Judge resolves a conflict between multiple agents.

    Args:
        action_description: What the action is
        agent_positions: {agent_id: their_position/conclusion}
        single_agent_response: What a lone agent would have done (for comparison)
    ...
    Returns:
        {
            ...
            "reversible": bool,
            "risk_flags": list[str],
            "judge_votes": {agent_id: "for"|"against"|"abstain"},
            "swarm_unanimous": bool,
        }
    """

    # Step 1: Hard checks (deterministic, never LLM)
    ...
    # Step 2: Get agent vote summary
    positions_text = "\n".join([
        f"[{agent_id}]: {pos[:200]}"
        for agent_id, pos in agent_positions.items()
    ])

    # Step 3: LLM evaluates legitimacy (not stakes/irreversibility — that's the guardrail's job)
    ...
    # Step 4: Determine final verdict combining judge + guardrail
    # Guardrail rule: irreversible OR high_stakes MUST escalate even with judge approval
    if judge_verdict == "approve":
        if irreversible or high_stakes or risk_flags:
            ...
            judge_confidence = min(judge_confidence, 0.5)  # Lower confidence when guardrail fires
        else:
