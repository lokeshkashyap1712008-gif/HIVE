...
class WebScout:
    """Scans websites, calls APIs, handles sessions and cookies."""

    async def run(description: str, context: dict = None) -> dict:
        """
        Execute a web-related task.
        description: Natural language description of what to do.
        Returns: dict with findings, URLs, status, etc.
        """
        ...
        try:
            ...
            if not url:
                # LLM: extract target from description
...
async def _fetch_page(url: str) -> dict:
    """Fetch a page and return title + content summary."""
    async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
        resp = await client.get(url)
        resp.raise_for_status()
        content = resp.text[:2000]  # first 2KB
...
async def _health_check(url: str) -> dict:
    """Check if a URL is responding and get basic info."""
...
async def _crawl_site(url: str, description: str) -> dict:
    """Crawl a site for links and pages."""
    import re
    ...
    links = list(set(links))[:20]  # dedupe, limit 20
...
def _extract_url(description: str) -> Optional[str]:
    """Use LLM to extract URL from description."""
    import re
    url = re.search(r"https?://[^\s]+", description)
    return url.group(0).rstrip(".,;") if url else None


# Module-level run function for HiveCore dynamic import
async def run(description: str, context: dict = None) -> dict:
