...
# Agent personas for the debate council
DEBATE_AGENTS = {
    "proposer": {
        "role": "Proposer",
...
def _format_finding(agent_role: str, finding: str) -> str:
    """Format a finding using Inverted Pyramid (conclusion first)."""
    return f"[{agent_role.upper()}]\nFINDING: {finding}"


async def _run_round1_individual(task: str, agents: list[str]) -> dict[str, str]:
    """Round 1: Each agent analyzes independently, no knowledge of others' views."""
    tasks = []
...
async def _run_round2_cross_debate(task: str, round1_findings: dict[str, str], agents: list[str]) -> dict[str, str]:
    """Round 2: Each agent sees others' findings and responds/challenges."""
    findings_text = "\n\n".join([
        f"[{DEBATE_AGENTS[a]['role']}]: {f}"
        for a, f in round1_findings.items()
    ])
...
async def _run_round3_refinement(task: str, round1: dict, round2: dict, agents: list[str]) -> dict[str, str]:
    """Round 3: Refine positions based on the debate."""
    all_debate_text = "\n\n".join([
...
async def _run_round4_negotiation(task: str, round3_findings: dict[str, str], agents: list[str]) -> dict:
    """Round 4: Reach consensus or escalate. Returns verdict + reasoning."""
    findings_text = "\n\n".join([
        f"[{DEBATE_AGENTS.get(a, {}).get('role', a)}]: {f}"
        for a, f in round3_findings.items()
    ])

    # Summarize debate and get verdict
    result = await chat(
        [
    ...
    # Count who agreed vs dissented
    votes = {a: "agreed" for a in agents}
    ...
    return {
        ...
        "debate_rounds": {
            "round_1_individual": round3_findings,  # Store R3 as final findings
...
async def run_debate(task: str, agent_ids: Optional[list[str]] = None) -> dict:
    """
    Run a full 4-round structured debate on a task.

    Args:
        task: The action/proposal to debate
        agent_ids: List of agents to include (default: all 4)
