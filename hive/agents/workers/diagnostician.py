...
def _get_recommendations(errors: list[str]) -> list[str]:
    recommendations = {
        "404": ["Check URL path is correct", "Verify resource exists on server"],
        "500": ["Check server logs", "Verify backend service is running"],
        "timeout": ["Increase timeout value", "Check network latency"],
        "permission denied": ["Check file permissions", "Verify service account has access"],
        "out of memory": ["Add more RAM", "Optimize memory usage"],
        "module not found": ["Install missing dependency", "Check PYTHONPATH"],
    }
    result = []
    for err in errors:
